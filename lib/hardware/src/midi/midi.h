...
class MIDI {
public:
  void note_off(uint8_t note, uint8_t ch = 0, uint8_t velocity = 0x40) { _send_midi(NoteOff | ch, note, velocity); }
  void note_on(uint8_t note, uint8_t ch = 0, uint8_t velocity = 0x40) { _send_midi(NoteOn | ch, note, velocity); }
