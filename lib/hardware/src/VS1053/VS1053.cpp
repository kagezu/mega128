...
void VS1053::init()
{
  ...
  _spi->init(1000); // Инициализируем на частоте не выше 12/7 MHz
  delayMicroseconds(10000);
  ...
  delayMicroseconds(200); // Длительность установки множителя 1200 XTALI
...
uint8_t VS1053::VSLoadUserCode(char *fileName)
{
  ...
  while (1) {
    ...
    if (n.word & 0x8000U) { /* RLE run, replicate n samples */
      ...
      while (n.word--) {
        write_register(addr.word, val.word);
      }
    }
...
uint8_t VS1053::enableTestSineWave(uint8_t freq)
{
  ...
  uint16_t MP3SCI_MODE = read_register(SCI_MODE);
  if (MP3SCI_MODE & SM_TESTS) {
    return 2;
  }

  write_register(SCI_MODE, MP3SCI_MODE | SM_TESTS);
...
uint8_t VS1053::disableTestSineWave()
{
  ...
  write_register(SCI_MODE, read_register(SCI_MODE) & ~SM_TESTS);
...
uint16_t VS1053::memoryTest()
{
  ...
  uint16_t MP3SCI_HDAT0 = read_register(SCI_HDAT0);
...
//------------------------------------------------------------------------------
/**
 * \brief Overload function of VS1053::set_volume(leftchannel, rightchannel)
 *
 * \param[in] data packed with left and right master volume
 *
 * calls VS1053::set_volume expecting the left channel in the upper byte
 ...
 *
 * calls VS1053::set_volume placing the input into both the left channel
 * and right channels.
 *
 * As specified by Data Sheet Section 8.7.11
 */
 ...
 */
void VS1053::set_volume(uint8_t leftchannel, uint8_t rightchannel)
{

  VolL = leftchannel;
  VolR = rightchannel;
  write_register(SCI_VOL, leftchannel, rightchannel);
}
...
{
  uint16_t MP3SCI_VOL = read_register(SCI_VOL);
  return MP3SCI_VOL;
}
// @}
// Volume_Group
...
uint16_t VS1053::getTrebleFrequency()
{
  ...
  sci_base_value.word = read_register(SCI_BASS);
...
void VS1053::setTrebleFrequency(uint16_t frequency)
{
  ...
  write_register(SCI_BASS, sci_base_value.word);
}
...
void VS1053::setEarSpeaker(uint16_t EarSpeaker)
{
  ...
  write_register(SCI_MODE, MP3SCI_MODE);
}
// @}
// EarSpeaker_Group
...
uint8_t VS1053::playMP3(char *fileName, uint32_t timecode)
{
  ...
  write_register(SCI_DECODE_TIME, 0); // Reset the Decode and bitrate from previous play back.
...
uint8_t VS1053::skip(int32_t timecode)
{
  ...
  if (isPlaying() && digitalRead(MP3_RESET)) {
    ...
    write_register(SCI_VOL, 0xFE, 0xFE);
    //seeked successfully
...
uint32_t VS1053::currentPosition()
{
  ...
  return(read_register(SCI_DECODE_TIME) << 10); // multiply by 1024 to convert to milliseconds.
}
...
void VS1053::getAudioInfo()
{
  ...
  //disable interupts
  // already disabled in read_register function
  //pauseDataStream();
  ...
  uint16_t MP3HDAT1 = read_register(SCI_HDAT1);
  Serial.print(F("0x"));
  Serial.print(MP3HDAT1, HEX);

  uint16_t MP3HDAT0 = read_register(SCI_HDAT0);
  Serial.print(F("\t0x"));
  ...
  uint16_t MP3Mode = read_register(SCI_MODE);
  Serial.print(F("\t0x"));
  ...
  uint16_t MP3Clock = read_register(SCI_CLOCKF);
  Serial.print(F("\t0x"));
  ...
  uint16_t MP3SCI_DECODE_TIME = read_register(SCI_DECODE_TIME);
  Serial.print(F("\t\t"));
...
{
  if (read_register(SCI_STATUS) & SS_VU_ENABLE) {
    return 1;
  }
  return 0;
}
...
int8_t VS1053::setVUmeter(int8_t enable)
{
  ...
  if (enable) {
    write_register(SCI_STATUS, MP3Status | SS_VU_ENABLE);
  }
  else {
    write_register(SCI_STATUS, MP3Status & ~SS_VU_ENABLE);
  }
...
{
  return read_register(SCI_AICTRL3);
}
...
//------------------------------------------------------------------------------
/**
 * \brief uint16_t Overload of VS1053::write_register
 *
 ...
 * write_register as to be written to the addressed VSdsp's registers.
 */
void VS1053::write_register(uint8_t addressbyte, uint16_t data)
{
  union twobyte val;
  val.word = data;
  write_register(addressbyte, val.byte[1], val.byte[0]);
}
 ...
 */
void VS1053::write_register(uint8_t addressbyte, uint8_t highbyte, uint8_t lowbyte)
{
 ...
 */
uint16_t VS1053::read_register(uint8_t addressbyte)
{
  union tbyte result;
  if (!X_RESET(GET)) return 0;
  while (!X_DREQ(GET));
...
uint16_t VS1053::Mp3ReadWRAM(uint16_t addressbyte)
{
  ...
  write_register(SCI_WRAMADDR, addressbyte);
  tmp1 = read_register(SCI_WRAM);

  write_register(SCI_WRAMADDR, addressbyte);
  tmp2 = read_register(SCI_WRAM);
...
void VS1053::flush_cancel(flush_m mode)
{
  ...
  for (int n = 0; n < 64; n++) {
    //    write_register(SCI_MODE, SM_LINE1 | SM_SDINEW | SM_CANCEL); // old way of SCI_MODE WRITE.
    write_register(SCI_MODE, (read_register(SCI_MODE) | SM_CANCEL));
    ...
    int cancel = read_register(SCI_MODE) & SM_CANCEL;
    if (cancel == 0) {
...
//  write_register(SCI_MODE, SM_LINE1 | SM_SDINEW | SM_RESET); // old way of SCI_MODE WRITE.
  write_register(SCI_MODE, (read_register(SCI_MODE) | SM_RESET));  // software reset. but vs_init will HW reset anyways.
...
uint8_t VS1053::ADMixerLoad(char *fileName)
{
  ...
  write_register(SCI_MODE, (read_register(SCI_MODE) | SM_LINE1));
#else
  write_register(SCI_MODE, (read_register(SCI_MODE) & ~SM_LINE1));
#endif
  playing_state = ready;
...
void VS1053::ADMixerVol(int8_t ADM_volume)
{
  ...
  MP3AIADDR.word = read_register(SCI_AIADDR);

  if ((ADM_volume > -3) || (-31 > ADM_volume)) {
    // Disable Mixer Patch
    MP3AIADDR.word = 0x0F01;
    write_register(SCI_AIADDR, MP3AIADDR.word);
  }
  else {
    // Set Volume
    //MP3AICTRL0.word = read_register(SCI_AICTRL0);
    MP3AICTRL0.byte[1] = (uint8_t)ADM_volume; // upper byte appears to have no affect
    MP3AICTRL0.byte[0] = (uint8_t)ADM_volume;
    write_register(SCI_AICTRL0, MP3AICTRL0.word);
