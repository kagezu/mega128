...
// потом постепенно номер секции увеличивается, и функция main выполняется в секции 10.
// Параметры стека задаются в секции 2, а до этого стек еще не определен.
#define GCC_INIT(	X ) __attribute__ ((naked, section (".init"#X  )))
...
#define GCC_FINI(	X ) __attribute__ ((naked, section (".fini" #X )))
